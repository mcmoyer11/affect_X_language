---
title: 'affect proj pilot: preprocessing'
author: "morgan moyer"
date: "`r Sys.Date()`"
...
# sanity check
nrow(cv) + nrow(vc) == nrow(d)

d$ID = as.factor(d$ID)
length(unique(d$ID)) # 65
...
# Add RT measure Column
results <- d %>%
  filter((Label %in% c("test_val", "test_conc")) & (Parameter == "Selection" | Value == "Start")) %>%
...
d <- d %>% 
    ...
    select(ID.true,Word,Label,ConcValCombo,Task,BlockOrder,Response,Accuracy,EventTime,Value,RT) %>% 
  distinct()
